...
while not matrix_ouccupied(player_board, rows, cols) and not matrix_ouccupied(opponent_board, rows, cols):
    ...
    player_board = place_number(player_board, rows, cols, player_dice) # player place the dice
    opponent_board = match_and_zero(player_board, opponent_board, cols) # the oppoent's dice may be removed 
    opponent_dice = roll_dice() # opponent roll the dice 
    opponent_board = place_number(opponent_board, rows, cols, opponent_dice) # opponent place the dice 
...
x = range(1000)
np.savetxt('strategy_0_output.csv', games_win, delimiter=',')
